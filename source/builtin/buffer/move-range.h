...
void bufferMoveRange(Buffer* buffer, long origin, long count, long destiny) // one base index
{
    if (count < 1) { return; }
    
    // destiny
    long destinyStart = destiny - 1; // adjusts to zero base index
    ...
    // origin    
    long originStart = origin - 1; // adjusts to zero base index
